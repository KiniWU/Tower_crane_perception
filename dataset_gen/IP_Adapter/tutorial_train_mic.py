...
def collate_fn(data):
    ...
    mic_clip_images = torch.cat([example["mic_clip_image"] for example in data], dim=0)
    drop_image_embeds = [example["drop_image_embed"] for example in data]

    return {
        "images": images,
        "text_input_ids": text_input_ids,
        "mic_clip_images": mic_clip_images,
    ...
    for epoch in range(0, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(ip_adapter):
                ...
                with torch.no_grad():
                    image_embeds = image_encoder(batch["mic_clip_images"].to(accelerator.device, dtype=weight_dtype)).image_embeds
                image_embeds_ = []
